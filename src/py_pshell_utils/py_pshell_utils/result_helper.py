...
import functools
import traceback
from typing import Any, TypeVar, Callable, Generic, Optional, Dict, List, Union, Type, cast
from loguru import logger as loguru_logger
from result import Result, Ok, Err

from py_pshell_utils.errors import PowerShellError
...
class ResultHandler:
    ...
    def from_function(func: Callable[..., T], *args, **kwargs) -> Result[T, PowerShellError]:
        ...
        Returns:
            Result型
        """
        loguru_logger.debug(f"Executing function {func.__name__} with Result wrapper")
        try:
            return Ok(func(*args, **kwargs))
        except PowerShellError as e:
            loguru_logger.debug(f"PowerShell error caught in ResultHandler: {e}")
            return Err(e)
            ...
            loguru_logger.debug(f"General exception caught in ResultHandler: {e}")
    ...
    def from_async_function(async_func: Callable[..., Any], *args, **kwargs) -> Result[Any, PowerShellError]:
        ...
        loguru_logger.debug(f"Executing async function {async_func.__name__} with Result wrapper")
        try:
            # イベントループを取得
            try:
            ...
            loguru_logger.debug(f"PowerShell error caught in async ResultHandler: {e}")
            return Err(e)
        except Exception as e:
            # 一般的な例外をPowerShellErrorに変換
            ...
            loguru_logger.debug(f"General exception caught in async ResultHandler: {e}")
    ...
    def handle_error(result: Result[T, E], default_value: Optional[T] = None) -> T:
        ...
        loguru_logger.debug(f"Error handled with default value: {result.unwrap_err()}")
        return default_value
    
    @staticmethod
    def unwrap_or_log(result: Result[T, E], logger_instance: Any = None) -> Optional[T]:
        """
        ...
        Returns:
            Okならその値、ErrならNone
        """
        log = logger_instance or loguru_logger
