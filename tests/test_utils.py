...
class MockPowerShellController(PowerShellControllerProtocol):
    """
    テスト用のモックPowerShellコントローラー
    
    指定されたレスポンスを返すモックコントローラーです。
    """
    ...
    def __init__(
        ...
        Args:
            ...
            default_response: コマンドマッピングにないコマンドのデフォルト応答
            raise_on_unknown: マッピングにないコマンドでエラーを発生させるかどうか
        """
    ...
    def execute_command_result(self, command: str, timeout: Optional[float] = None):
        """
        コマンドを実行しResult型で結果を返します（モック）
        """
        ...
        if result.success:
            ...
            return Err(PowerShellExecutionError(result.error, command))
    ...
    def execute_script_result(self, script: str, timeout: Optional[float] = None):
        """
        スクリプトを実行しResult型で結果を返します（モック）
        """
        return self.execute_command_result(script, timeout)
    ...
    def set_environment_variable(self, name: str, value: str) -> None:
        ...
        環境変数を設定します（モック）
        """
        self.execute_command(f"$env:{name} = '{value}'")
